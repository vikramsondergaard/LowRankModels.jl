...
function test(A::AbstractArray, losses::Array{Loss, 1}, s::Int64, k::Int64)
    ...
    for scale in scales
        println("Fitting orthogonal fair GLRM with scale=$scale")

        fglrm = FairGLRM(A, losses, SoftOrthogonalReg(scale, normalise(A[:, s])), ZeroReg(), k, s,
        ...
        println("Orthogonality penalty (without scaling) is $total_orthog")
    end
end
