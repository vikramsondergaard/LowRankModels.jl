...
function fit!(glrm::FairGLRM, params::ProxGradParams;
        ...
        verbose=true,
        kwargs...)
    println("Starting multi-threaded proxgrad")
    ### initialization
    A = glrm.A # rename these for easier local access
    losses = glrm.losses
    rx = glrm.rx
    ...
    for i=1:params.max_iter
        println("Starting iteration $(i) of multi-threaded proxgrad")
        # STEP 1: X update
        # XY = X' * Y was computed above
        ...
        for inneri=1:params.inner_iter_X
            println("Starting X iteration $(inneri) of multi-threaded proxgrad")
            ...
            Threads.@threads for k_prime=1:k # new section that I've added for component-wise gradient
