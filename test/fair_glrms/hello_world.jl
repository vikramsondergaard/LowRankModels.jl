...
protected_char = size(A, 2)
num_protected_categories = 3

groups = partition_groups(A, protected_char)

fglrm = FairGLRM(A, losses, regularizers, QuadReg(), 2, protected_char,
    WeightedLogSumExponentialLoss(1, [length(groups[i]) / m for i=1:num_protected_categories]))
...
fglrm = FairGLRM(A, losses, regularizers, QuadReg(), 2, protected_char,
    WeightedLogSumExponentialLoss(1, [length(groups[i]) / m for i=1:num_protected_categories]), obs=Ω)
fit!(fglrm, verbose=true)
...
# glrm = FairGLRM(DataFrame(A), 3, data_types)
# fit!(glrm, verbose=false)
...
# fglrm = FairGLRM(sparseA, QuadLoss(), QuadReg(), QuadReg(), 5, protected_char,
#     WeightedLogSumExponentialLoss(1, [length(groups[i]) / m for i=1:num_protected_categories]))
# fit!(glrm, verbose=false)
