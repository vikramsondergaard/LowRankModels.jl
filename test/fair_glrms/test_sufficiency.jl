...
function test(A::AbstractArray, losses::Array{Loss, 1}, s::Int64, k::Int64, y::Int64; is_categorical::Bool=false)
    ...
    for scale in scales
        ...
        fglrm = FairGLRM(A, losses, SufficiencyReg(scale, A[:, s], separator), ZeroReg(), k, s,
        ...
        println("Sufficiency penalty (without scaling) is $total_orthog")
    end
end
...
function test_large()
    ...
    total_orthog = sum(evaluate(SufficiencyReg(1.0, A₄[:, s], A₄[:, y]), glrmX[i, :]) for i=1:k)
    println("Sufficiency penalty (without scaling) is $total_orthog")
