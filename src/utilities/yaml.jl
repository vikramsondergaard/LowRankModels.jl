...
function parse_losses(losses::Vector{Dict{Any, Any}})
    ...
    bool_losses = [get_loss(l) for l in losses[2]["boolean_losses"]]
    categorical_losses = [get_loss(l["loss"], n_classes=l["n_classes"], bl=l["bin_loss"]) for l in losses[3]["categorical_losses"]]
    ordinal_losses = [get_loss(l["loss"], n_classes=l["n_classes"]) for l in losses[4]["ordinal_losses"]]
